...
![Version: 0.2.0](https://img.shields.io/badge/Version-0.2.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.45.0](https://img.shields.io/badge/AppVersion-0.45.0-informational?style=flat-square) 

Installs Strimzi, Drain Cleaner, Schema Registry, and a Kafka cluster in KRaft mode.
...
|------------|------|---------|
| file://charts/cfk/ | schemaregistry_operator(confluent-for-kubernetes) | 0.1145.6 |
...
| schemaregistry.affinity | object | `{}` | affinity for pod scheduling. Reference [Assign Pods to Nodes using Node Affinity](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pods-nodes-using-node-affinity). |
| schemaregistry.annotations | object | `{}` | annotations to be added to the SchemaRegistry resource. |
| schemaregistry.bootstrapPort | int | `9094` | bootstrapPort is the internal listener port of the Kafka cluster protected by mTLS. This only needs to be changed if not using the default ports, or if using no authentication for development purposes. |
| schemaregistry.dashboards.enabled | bool | `false` | Indicates whether or not to deploy a Schema Registry Grafana dashboards that will be imported automatically. |
| schemaregistry.dashboards.namespace | string | `"monitoring"` | namespace is the namespace where the Grafana dashboards will be deployed. This should be the same namespace as the Prometheus Operator and Grafana instance. |
| schemaregistry.envVars | list | `[]` | envVars are additional environment variables to set. |
| schemaregistry.extraConfigOverrides | object | `{}` | extraConfigOverrides are additional configuration overrides for server, JVM, and Log4j properties. Reference: [Schema Registry Configuration Reference](https://docs.confluent.io/platform/current/schema-registry/installation/config.html). |
| schemaregistry.image.application | string | `"confluentinc/cp-schema-registry:7.8.0"` | application is the image that provides a centralized repository for managing and validating schemas in Kafka message data, facilitating consistent serialization and deserialization across applications. It ensures data compatibility and quality by allowing producers and consumers to enforce schema compliance, thereby maintaining data integrity as schemas evolve.  |
| schemaregistry.image.init | string | `"confluentinc/confluent-init-container:2.10.0"` | init is the image used in Kubernetes deployments of Confluent Platform to prepare the environment before the main application starts. It ensures proper initialization and configuration, and its version should match the Confluent for Kubernetes (CFK) version being deployed. |
| schemaregistry.ingress.className | string | `"nginx"` | className is the name of the Ingress class. |
| schemaregistry.ingress.enabled | bool | `true` | Indicates whether or not an Ingress resource is created. |
| schemaregistry.ingress.hosts[0].host | string | `""` | host is the hostname of a request that must match exactly or use a wildcard as the subdomain. |
| schemaregistry.ingress.hosts[0].paths[0].path | string | `"/"` | path is part of a list of one or more paths that are associated with a backend service. |
| schemaregistry.ingress.hosts[0].paths[0].pathType | string | `"Prefix"` | pathType is a field that can specify how Ingress paths should be matched. Reference [Path types](https://kubernetes.io/docs/concepts/services-networking/ingress/#path-types). |
| schemaregistry.labels | object | `{}` | labels to be added to the SchemaRegistry resource. |
| schemaregistry.rackTopology.customKey | string | `""` | customKey allows to override the standard `topology.kubernetes.io/zone` key used for the rack-aware feature. |
| schemaregistry.rackTopology.enabled | bool | `true` | Indicates whether or not to enable the rack-aware feature for the schema registry to improve resiliency, availability, and reliability. This will distribute the instances across the different availability zones or actual racks in the data center. When testing locally, set this to `false`. |
| schemaregistry.rackTopology.maxSkew | int | `1` | maxSkew is the maximum degree to which pods may be unevenly distributed. Setting 0 is not allowed. |
| schemaregistry.replicas | int | `1` | replicas is for the number of Schema Registry instances. Leader election is enabled for more than one replica. |
| schemaregistry.replicationFactor | int | `3` | replicationFactor is the number of replicas for the Schema Registry. This should be set to `1` for development purposes, and `3` for production. |
| schemaregistry.resources | object | `{}` | Optionally request and limit how much CPU and memory (RAM) the container needs. Reference [Resource Management for Pods and Containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers). |
| schemaregistry.tls.tls_crt | string | `""` | tls_crt is the trusted TLS certificate in PEM format. Required when both `schemaregistry_operator.managedCerts.enabled` and `schemaregistry.tls.useAutoGeneratedCert` are `false`. Passed value will be auto base64 encoded. |
| schemaregistry.tls.tls_key | string | `""` | tls_key is the trusted TLS private key in PEM format. Required when both `schemaregistry_operator.managedCerts.enabled` and `schemaregistry.tls.useAutoGeneratedCert` are `false`. Passed value will be auto base64 encoded. |
| schemaregistry.tls.useAutoGeneratedCert | bool | `false` | useAutoGeneratedCert indicates whether or not to enable auto-generated certificates. If set to `true` and `schemaregistry_operator.managedCerts.enabled` is set to `false`, then the legacy approach of creating a secret called `ca-pair-sslcerts` with a self-signed CA cert and key (e.g., strimzi cluster CA) will be needed to do the same. Set both this and `schemaregistry_operator.managedCerts.enabled` to `false` to use a trusted TLS certificate. |
| schemaregistry.tolerations | list | `[]` | tolerations allow the scheduler to schedule pods onto nodes with matching taints. Reference [Taints and Tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration). |
| schemaregistry_operator.enabled | bool | `false` | Indicates whether or not to deploy Schema Registry from Confluent for Kubernetes (CFK) with the Kafka cluster. Requires an internal listener that is protected by mTLS. |
| schemaregistry_operator.managedCerts.caCertificate.secretRef | string | `"ca-keypair-for-generated-tls"` | secretRef is the name of the tls secret holding a CA cert and key defined as `tls.crt` and `tls.key`. |
| schemaregistry_operator.managedCerts.enabled | bool | `false` | Indicates whether or not to enable auto-generated certificates using the referenced tls secret with a self-signed CA cert and key (e.g., strimzi cluster CA). If set to `false` and `schemaregistry.tls.useAutoGeneratedCert` is set to `true`, then the legacy approach of creating a secret called `ca-pair-sslcerts` with a self-signed CA cert and key (e.g., strimzi cluster CA) will be needed to do the same. |
| schemaregistry_operator.replicas | int | `1` | replicas is for the number of Schema Registry Operator instances. |
...
| strimzi-drain-cleaner.secret.ca_bundle | string | `""` | ca_bundle is the CA certificate bundle in PEM format used for the ValidatingWebhookConfiguration regardless of the `strimzi-drain-cleaner.secret.create` state. Required when `strimzi-drain-cleaner.certManager.create` is `false`. Passed value must be pre-base64 encoded. |
...
| strimzi-drain-cleaner.secret.tls_crt | string | `""` | tls_crt is the TLS certificate in PEM format used for the ValidationWebhook. Required when `strimzi-drain-cleaner.certManager.create` is `false`. Passed value must be pre-base64 encoded. |
| strimzi-drain-cleaner.secret.tls_key | string | `""` | tls_key is the TLS private key in PEM format used for the ValidationWebhook. Required when `strimzi-drain-cleaner.certManager.create` is `false`. Passed value must be pre-base64 encoded. |
...
| testResources.create | bool | `false` | Indicates whether or not to create the test resources, which consist of a KafkaUser, KafkaTopic, and a AVRO schema if `schemaregistry_operator.enabled is set to `true`. |
